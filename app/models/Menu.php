...
class Menu extends Eloquent {
    ...
    public function getMenuOptions() {
        ...
        $menuOptions = array(
            'General'       => array(
                ...
                'blog'    => 'Blog',
                'angebot'    => 'Angebot',
                'sofunktioniertes'    => 'So funktioniert es',
                'kochabobox'    => 'Kochabobox',
                'obstbox'    => 'Obstbox',
                'beispielbox'    => 'Beispielbox',
    ...
    public function generateFrontMenu($menu, $parentId = 0, $starter = false) {
        ...
        $result = null;
  $i = 1;  // Durchzählen der Mnüelemente für Ausgabe
        ...
        foreach ($menu as $item) {
            ...
            if ($item->parent_id == $parentId) {
                ...
                $childItem = $this->hasChildItems($item->id);
                             ...
                             $result .= "<li  id ='' class='menu-item  " . (($childItem) ? 'dropdown' : null) . (($childItem && $item->parent_id != 0) ? ' dropdown-submenu' : null)   . "'>
                                <a  class='para' id ='para' data-tab='" . $i . "' href='" . url($item->url) . "'" . (($childItem) ? 'class="dropdown-toggle" data-toggle="dropdown"' : null) . ">{$item->title}" . (($childItem && $item->parent_id == 0) ? '<b class="caret"></b>' : null) . "</a>" . $this->generateFrontMenu($menu, $item->id) . "
                            </li>";       
...
<?php         
           ...
           $i++; // Durchzählen der Mnüelemente für Ausgabe
        }

        return $result ? "\n<ul class='" . (($starter) ? ' nav navbar-nav navbar-left ' : null) . ((!$starter) ? ' dropdown-menu ' : null) . "'>\n$result</ul>\n" : null;
    }
