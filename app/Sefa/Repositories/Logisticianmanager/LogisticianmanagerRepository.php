...
public function all() {
return $this->logisticianmanager->orderBy('created_at', 'DESC')
...
if($all)
return $this->logisticianmanager->orderBy('created_at', 'DESC') ->where('id', '>', 1)
->paginate(($perPage) ? $perPage : $this->perPage);
return $this->logisticianmanager->orderBy('created_at', 'DESC')
->where('is_published', 1) ->where('id', '>', 1)
->paginate(($perPage) ? $perPage : $this->perPage);
}
...
$affectedRows_update = Deliveryzipcode::where('user_id', '=', $id)->update(array('user_id' => '0'));
...
return Response::json(array('result' => 'success', 'changed' => ($logisticianmanager->is_published) ? 1 : 0));
}
