...

{
    ...
    {
        return $this->address->orderBy('created_at', 'DESC')->where('is_published', 1)->get();
    }
    ...
    {
        if ($all) return $this->address->orderBy('created_at', 'DESC')->paginate(($perPage) ? $perPage : $this->perPage);
        return $this->address->orderBy('created_at', 'DESC')->where('is_published', 1)->paginate(($perPage) ? $perPage : $this->perPage);
    }
...

    {
        ...
        return Response::json(array(
            'result' => 'success',
            'changed' => ($address->is_published) ? 1 : 0
        ));
    }
}
