...

{
    ...
    {
        return $this->aktuellerezepte->orderBy('created_at', 'DESC')->where('is_published', 1)->get();
    }
    ...
    {
        if ($all) return $this->aktuellerezepte->orderBy('created_at', 'DESC')->paginate(($perPage) ? $perPage : $this->perPage);
        return $this->aktuellerezepte->orderBy('created_at', 'DESC')->where('is_published', 1)->paginate(($perPage) ? $perPage : $this->perPage);
    }
...

    {
        ...
        return Response::json(array(
            'result' => 'success',
            'changed' => ($aktuellerezepte->is_published) ? 1 : 0
        ));
    }
}
