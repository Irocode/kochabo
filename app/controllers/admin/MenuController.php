...

{
...

    {
        ...
        if (isset($urlInfo['host'])) $url = ($host == $urlInfo['host']) ? $urlInfo['path'] : $formData['url'];
        else $url = ($formData['type'] == 'module') ? $formData['url'] : "" . $formData['url'];
        $this->menu->url = $url;
        $this->menu->save();
        Notification::success('Menüelement erfolgreich hinzugefügt');
...

    {
        ...
        $this->menu->url = $url;
        $this->menu->save();
        Notification::success('Menüelement erfolgreich geupdatet');
        return Redirect::route('admin.menu.index');
    }
    /**
...

    {
        ...
        $this->menu = $this->menu->find($id);
        $this->menu->delete();
        Notification::success('Menüelement gelöscht');
        return Redirect::action('App\Controllers\Admin\MenuController@index');
    }
...

    {
        ...
        return Response::json(array(
            'result' => 'success',
            'changed' => ($this->menu->is_published) ? 1 : 0
        ));
    }
}
