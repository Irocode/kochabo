...
"{{{ $user->updated_at->format('d-m-Y (H:i:s)')}}}", 
  "@if ($user->status != 'Suspended')<button class=\"btn-u btn-u-yellow\" type=\"button\" onClick=\"location.href='{{ route('suspendUserForm', array($user->id)) }}'\">Suspendieren</button>@else<button class=\"btn-u btn-u-yellow\" type=\"button\" onClick=\"location.href='{{ action('UseradminController@unsuspend', array($user->id)) }}'\">Entsperren</button>@endif",
