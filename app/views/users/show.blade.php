...
<div class="row servive-block margin-bottom-10">
   <div class="col-md-3 col-sm-6">
      <div class="servive-block-in">
         ...
         <p>{{ $user->produkttyp }} </p>
         @endif                 
         @if ($user->price_produkt) 
         <p>{{ $user->price_produkt }} € </p>
         @endif
         <hr>
         @if ($user->obstbox) 
         <p>{{ $user->obstbox }} @endif @if ($user->obstbox){{ $user->price_adobstbox }} €</p>
         @endif
         @if ($user->obstbox)  
         <hr>
         @endif
         @if ($user->wein) 
         <p>{{ $user->wein }}@endif @if ($user->wein) 
         <p>{{ $user->price_adwein }} €</p>
         @endif
         @if ($user->wein)  
         <hr>
         @endif
